...
- 当前非研判 Agent 发布候选的 Alembic 唯一 head 为 `20260808_0082`，与目标 ECS 最近一次只读确认的数据库 head 一致；本次发布不执行数据库迁移，应用自动迁移保持关闭。
- 报价资料研判 Agent 的新功能、API、配置、任务、契约及 `20260810_0083`—`20260811_0091` 数据库迁移只允许保留在独立本地开发环境；在用户明确确认“全部开发完成并允许上线”前，不得进入正式发布候选或 ECS。
- 任何后续数据库升级仍须先重新确认实际 head、完成全量备份、SHA-256 校验和恢复演练，再以同一版本发布 API/Worker。
