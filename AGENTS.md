...
- 原 Phase 3 执行任务/执行速度与 Phase 4a 会议纪要功能曾完成验证，已于 2026-07-31 按产品决定完整退役；前端入口、API、服务、模型、RBAC 模块和功能开关均已移除，Alembic `20260731_0078` 删除其 5 张专用表。`project_*` 项目进度与项目任务继续保留。
- 原 BIZ-1a 商务台账已于 2026-07-31 按产品决定完整退役；前端入口、API、服务、schema、RBAC 模块和功能开关均已移除。Alembic `20260731_0079` 删除 outbound 台账记录、`client_inquiry_events` 及台账专用字段，并把 `client_inquiries` 恢复为 Phase 2 响应速度追踪的纯 inbound 模型。
- 原“成本测算闭环”（COST-MEASURE-1 / COST-MEASURE-2）已于 2026-07-31 按产品决定完整退役；仅移除 `cost_measurement*` 前后端链路、RBAC 模块、功能开关和三张专属表。Alembic `20260731_0080` 删除 `cost_measurements`、`cost_measurement_lines`、`cost_measurement_events`；项目报价、对话报价、成本库、企业/账户定额、智能组价、报价资料研判 Agent 和智能助手继续保留。
...
- 当前代码迁移 head：`20260731_0080`；正式报价成本与 RAG 源为 MySQL `cost_items.active`，旧 `materials` 已清空退役、`material_snapshots` 仅作旧审计回溯；执行系统专用表由 `20260731_0078` 退役，商务台账由 `20260731_0079` 退役，成本测算闭环三张专属表由 `20260731_0080` 退役，其他报价、成本库、项目进度、企业/账户定额、预算项目与 Agent 数据结构继续保留。
- 当前内网数据库已于 2026-07-31 在全量备份后升级到 `20260731_0080 (head)`；备份为 `AI_Middle_Office/backups/retired_modules_pre_0080_20260731_182547.sql`（759.31 MB，SHA-256 `732CEB33858B5E076BADBE975DD9868720E543D57E8E6394A751E66A9BAE12DF`）。FastAPI 已重启，`/health/ready=ready`，数据库/Celery 正常，退役入口 404，保留模块运行态 smoke 正常。
- 内网验证数据库升级到 head 前必须先备份；升级会删除执行系统 5 张专用表、商务台账 outbound 记录/事件表/专用字段，以及成本测算闭环 3 张专属表；不影响 inbound 客户咨询、响应速度、项目/对话报价、成本库、企业/账户定额、预算计价、Agent 或 `project_*` 项目进度表。
