...
readonly REPORT="/home/aiadmin/ai-ecs-post-reboot-validation.txt"
...
[[ "$(systemctl show -p NeedDaemonReload --value "$SERVICE")" == no ]]
...
[[ "$(iptables -w -t raw -S PREROUTING | grep -c -- '--comment ai-hybrid-')" -eq 3 ]]
[[ "$(iptables -w -S DOCKER-USER | grep -c -- '--comment ai-hybrid-')" -eq 2 ]]
...
echo "post_reboot_trace=$trace_report"
echo "post_reboot_validation=passed"
chmod 0644 "$REPORT"
