...
from app.services.dxf_table_field_convergence import (  # noqa: E402
    converge_table_fields,
    write_field_convergence_outputs,
)
from app.services.dxf_table_reconstructor import reconstruct_dxf_tables  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BIZ-2x-3 DXF 表格字段级收敛")
    ...
    parser.add_argument(
        ...
        help="字段收敛报告输出目录，默认 outputs/biz2x3",
    )
    parser.add_argument("--text-record-limit", type=int, default=DEFAULT_TEXT_RECORD_LIMIT, help="每个 DXF 保存文字记录上限")
    parser.add_argument("--row-limit-per-table", type=int, default=30, help="每个表格候选最多读取多少行")
    ...
    try:
        ...
        table_report = reconstruct_dxf_tables(parsed_files, row_limit_per_table=args.row_limit_per_table)
        report = converge_table_fields(table_report)
    except (DxfTextExtractionError, OSError) as exc:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "DXF_TABLE_FIELD_CONVERGENCE_FAILED",
                    "message": str(exc),
                },
                ensure_ascii=False,
                indent=2,
    ...
    if not args.no_write:
        ...
        outputs = write_field_convergence_outputs(
            report,
            args.output_dir,
            stem=f"BIZ2x3_DXF表格字段收敛_{timestamp}",
        )
