...
AI_ESTIMATE_ENGINE_VERSION = "budget-pricing-ai-estimate-p2-2c3-reference-context"
...
pricing_breakdown: object，税前单价拆分；必须包含 labor_unit_cost、main_material_unit_cost、auxiliary_material_unit_cost、main_material_without_loss、loss_rate、tax_amount
material_supply_mode: string，乙供、甲供或无主材
confidence: number，0 到 1
basis: string，简短说明估价依据
risks: string array，列出需要人工确认的风险

拆分规则：
1. unit_price 是不含税综合单价，不包含税金。
2. tax_amount 必须按 unit_price * 9% 计算，税率固定 9%。
3. 只有人工的项目，材料相关字段填 0，loss_rate 填 0，material_supply_mode 填“无主材”。
4. 有材料的项目，判断是否存在损耗；无损耗则 loss_rate 填 0。
"""
...
unit_price: number，大于 0，最多 6 位小数
pricing_breakdown: object，税前单价拆分；必须包含 labor_unit_cost、main_material_unit_cost、auxiliary_material_unit_cost、main_material_without_loss、loss_rate、tax_amount
material_supply_mode: string，乙供、甲供或无主材；只有人工的项目填“无主材”
confidence: number，0 到 1
basis: string
risks: string array
"""
...
def _draft_reference_context(draft: BudgetProjectPricingDraft) -> dict[str, Any]:
    source_snapshot = _json_load(draft.source_snapshot_json, {})
    context = source_snapshot.get("ai_reference_context") if isinstance(source_snapshot, dict) else None
    return context if isinstance(context, dict) else {}


def _line_summary_context(line: BudgetProjectPricingDraftLine) -> dict[str, Any]:
    source_row = _json_load(line.source_row_snapshot_json, {})
    standard = source_row.get("standard_row") if isinstance(source_row, dict) else None
    standard = standard if isinstance(standard, dict) else {}
    multiplier = _decimal(standard.get("budget_summary_multiplier")) or Decimal("1")
    if multiplier <= 0:
        multiplier = Decimal("1")
    quantity = _decimal(line.calculation_quantity) or Decimal("0")
    sources = standard.get("budget_summary_multiplier_sources")
    return {
        "summary_multiplier": _decimal_text(_q6(multiplier)),
        "effective_calculation_quantity": _decimal_text(_q6(quantity * multiplier)),
        "summary_sources": sources[:8] if isinstance(sources, list) else [],
    }
...
def _line_input_snapshot(draft: BudgetProjectPricingDraft, line: BudgetProjectPricingDraftLine) -> dict[str, Any]:
    return {
        ...
        "summary_context": _line_summary_context(line),
        "reference_context": _draft_reference_context(draft),
...
def _rule_estimate(snapshot: dict[str, Any], *, fallback_reason: str | None = None) -> dict[str, Any]:
    ...
    unit_price = _q6(price)
    return {
        "unit_price": _decimal_text(unit_price),
        "pricing_breakdown": _estimate_pricing_breakdown(snapshot, unit_price),
        "confidence": 0.35,
...
def _estimate_pricing_breakdown(
    snapshot: dict[str, Any],
    unit_price: Decimal,
    raw_breakdown: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if isinstance(raw_breakdown, dict) and any(key in raw_breakdown for key in _BREAKDOWN_UNIT_PRICE_KEYS):
        normalized = _normalize_pricing_breakdown(raw_breakdown)
        composite = _decimal(normalized.get("composite_unit_price"))
        if composite and composite > 0:
            scaled: dict[str, Any] = {}
            for key in _BREAKDOWN_UNIT_PRICE_KEYS:
                value = _decimal(normalized.get(key))
                if value is not None:
                    scaled[key] = _decimal_text(_q6(value * unit_price / composite))
            for key in ("main_material_without_loss", "loss_rate", "owner_material_unit_price", "owner_material_loss_amount", "material_supply_mode", "remark"):
                if key in normalized:
                    scaled[key] = normalized[key]
            return _normalize_pricing_breakdown(scaled)

    text = f"{snapshot.get('item_name') or ''} {snapshot.get('spec') or ''}"
    ...
    return _normalize_pricing_breakdown(breakdown)


def _clean_llm_estimate(payload: dict[str, Any], *, snapshot: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    raw_breakdown = payload.get("pricing_breakdown") if isinstance(payload.get("pricing_breakdown"), dict) else payload.get("cost_breakdown")
    source_snapshot = snapshot if isinstance(snapshot, dict) else payload
    return {
        "unit_price": _decimal_text(price),
        "pricing_breakdown": _estimate_pricing_breakdown(source_snapshot, price, raw_breakdown if isinstance(raw_breakdown, dict) else None),
...
async def _generate_estimate(snapshot: dict[str, Any], *, current_user: User) -> dict[str, Any]:
    ...
    estimate = _clean_llm_estimate(_extract_json_object(content), snapshot=snapshot)
    estimate.update(
        {
            "provider": "deepseek",
            "model": model,
...
def _batch_line_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "summary_context": snapshot.get("summary_context"),
    }
    ...
    *,
    ...
    user_payload = {
        ...
        "reference_context": next(
            (snapshot.get("reference_context") for snapshot in rows if snapshot.get("reference_context")),
            {},
        ),
        "boundary": {
            "result_scope": "mutable_pricing_draft_only",
    ...
    for item in items:
        ...
        if not row_id or row_id not in expected_ids or row_id in results:
            continue
        estimate = _clean_llm_estimate(item, snapshot=next(snapshot for snapshot in rows if _batch_row_id(snapshot) == row_id))
        estimate.update(
            {
                "provider": "deepseek",
                "model": model,
...
def apply_budget_pricing_ai_estimate_to_line(
    ...
    *,
    ...
    pricing_breakdown = estimate.get("pricing_breakdown") if isinstance(estimate.get("pricing_breakdown"), dict) else None
    if not pricing_breakdown:
        pricing_breakdown = _estimate_pricing_breakdown(input_snapshot, price)
        estimate["pricing_breakdown"] = pricing_breakdown
    line.pricing_breakdown_json = _json_dump(pricing_breakdown)
    snapshot_payload = {
