...
> 旗胜投标机会研判 Pure Agent 的 B01—B05、V601—V608、C01、C02-1—C02-4 与 C03-1—C03-4 已完成；C03-4 独立专项 `6 passed / 0 failed`（1.65 秒），C03/C02/C01 进程内 SQLite 相邻回归 `38 passed / 0 failed`（3.89 秒）。新 Action 会持久化 Profile/授权策略/Scope 恢复绑定；Controller 仅在 Registry、Authorization、Source Heads、Cancellation、Effect/Action/Artifact Hash 与 Budget Settlement 全部复验后消费原结果并继续动态 Action Loop。`retry_safe`、`reconcile` 首版只报告、不自动重放 Tool/模型/副作用；旧 Action 无恢复绑定、终态仅有 Hash 无结果正文、预算未结算均 fail closed。未新增迁移，隔离开发 head 仍为尚未应用且不得进入 ECS 的 `20260821_0110`；未调用模型、真实资料、RAG、真实 Tool、MCP、外部数据库、Milvus 或 ECS。下一开发目标待用户指定；B07 仍仅在“全部开发完成并允许上线”后展开。

> 报价资料研判 Agent Phase 4D-3 v0.1-r62 已完成真实业务闭环：在完全隔离副本中以资料包v3、香港中心307页真实PDF、RQ2-B/固定本地BCE和DeepSeek运行成功；Comparison Baseline 16槽为`1 supported / 6 partial / 9 unknown`，Run经一次真实失败和API-43 Checkpoint恢复后`27/27 Task`、`34 Model`、`23 Tool`、`95 Checkpoint`、Run Validation `52/52`通过，6 Claim/6 Atom引用，模型成本`19546` micro-USD。决策由历史合成数据`no_bid`纠正并稳定为真实证据下`insufficient/hold`，七项硬门均unknown；新RC `mvp-rc-20260818091440-c21d8d4b3bf9`以`accepted_with_follow_up`冻结。修复Executor旧`enterprise_snapshot_id`输出引用与新`comparison_baseline_id`不兼容，Phase4D-3专项`12 passed`。9017现为view-only，写请求403且数据库Hash不变；未调用OCR/视觉/外部MCP，9015/ECS未改，0108不得应用到ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4d3-fact-verification-comparability-20260818.md`。

> 报价资料研判 Agent Phase 4D-2 v0.1-r60 已完成真实企业资料包v3、Business Baseline与“香港中心”七项硬门复验：19个Evidence Item明确映射I01—I05，真实Snapshot按I01—I05 partial、I06—I11 unknown冻结，Business Baseline `enterprise-business-20260818030437-cb6c17be2ff6`为`verified_with_follow_up`。同一307页真实PDF以RQ2-B、本地BCE和DeepSeek V4 Flash重跑，27/27 Task、Run/Report/Validation成功，10 Claim/28条权威引用，成本约`$0.018733`。Decision由合成企业数据的`no_bid`纠正为`insufficient`：HG02 fail→unknown、HG04/HG07 pass→unknown，七项均unknown且无明确fail，避免用合成“合规清白/无禁投风险”或虚构资质形成过度确定结论。当前缺口是官方状态、合同近五年/履约/相似性、逐人证书与劳动关系、资金担保、合规及客户风险权威核验。9015已恢复view-only，Worker/模型关闭、写请求403；未调用OCR/视觉/外部MCP，未改ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4d2-enterprise-evidence-import-20260817.md`。

> 报价资料研判 Agent Phase 4D-1 v0.1-r59 已完成本地隔离收口：I01—I11逐槽业务核验、不可变Enterprise Business Baseline、Run Bootstrap Baseline Hash和历史RC→新Decision/HG01—HG07复验完成合同/Schema、0106升降级与0083—0106拓扑、核心及Phase4C/Preflight/API-41/SSE相邻回归，共`239 passed / 0 failed`。一次性9010动态验证冻结与幂等、两个Candidate Hash漂移409、Bootstrap版本/Hash权威绑定；合成确定性全链Run/Report/Validation成功，11槽显式unknown时七项硬门不误判且Decision为`insufficient`。浏览器Preflight零阻断、SSE/Trace/Report正常，缺历史RC/Atom权威的合成旧Run按设计fail-closed；9010已停止、9003未改。未使用真实企业数据/PDF/BCE/OCR/视觉/生成模型/外部MCP/ECS，head为0106、默认关闭且不得应用到ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4d1-business-baseline-decision-revalidation-20260817.md`。

> 报价资料研判 Agent Phase 4C-3 v0.1-r57 已完成首次真实 PDF 业务验收并实际冻结本地 MVP RC：“香港中心”307页真实 PDF 以 RQ2-B、固定本地 BCE 和 DeepSeek V4 Flash 跑通，Run/Report/Validation 成功，27 Task、88 Attempt、36 Model、25 Tool、99 Checkpoint，生成3 Claim/12 Atom 引用，账本成本约`$0.026401`。RC 14项系统检查全通过、Atom-only违规0，以`accepted_with_follow_up`冻结为`mvp-rc-20260817084759-f77d02eded07`；幂等重放、view-only 403与浏览器禁写均通过，本轮直接专项`31 passed / 0 failed`。一次 Search Child 混入候选的真实失败被权威层正确拒绝，已以不放宽 Atom 证据门的 Gateway 过滤修复。当前企业快照仍为隔离演示数据，Parse 84分/`review_required`且未做OCR/视觉，因此`no_bid`不是真实投标决策；未调用外部MCP/生产Milvus/ECS，head保持0105，不得应用到ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4c3-real-business-acceptance-mvp-rc-20260817.md`。

> 报价资料研判 Agent Phase 4C-2 v0.1-r54 已完成本地隔离收口：零持久化 Baseline Validate、来源/有效期/partial/unknown、Diff/稳定Hash、Candidate Hash漂移围栏和HG01—HG07 Acceptance已接入Runtime Lab。合同/Schema、核心、Phase4C-1/Preflight/API-41/SSE相邻及0083—0104迁移拓扑自动矩阵共`191 passed / 0 failed`。一次性9005 execute动态验证预览不落库、错误Hash 409、正确Hash冻结与幂等重放、11/11槽ready、HG02—HG07 ready且HG01 deferred_tender；浏览器验证候选变化立即使冻结失效。同库view-only的Worker/模型/写权限关闭，两个写接口403、历史快照可读、前端写按钮禁用且控制台0 error；9005已停止、9003未改动。无新迁移，唯一head保持0104，默认关闭、不改旧`bid_intake_*`、不得应用到ECS；未使用真实PDF/OCR/视觉/Embedding/Reranker、生成模型/外部MCP或外部环境。详见 `AI_Middle_Office/docs/bid-assessment-phase4c2-enterprise-baseline-acceptance-20260817.md`。

> 报价资料研判 Agent Phase 4C-1 v0.1-r52 已完成本地隔离业务闭环与专项验证：I01—I11不可变企业能力快照、0104企业事实血缘、P1确定性Skill、HG01—HG07、Decision/Claim/Report、Preflight v2和Runtime Lab双模式均已接通。最终自动矩阵`31 passed / 0 failed`；两次合成TXT全链均27/27 Task并成功收敛，partial/过期数据按预期保持partial/unknown。view-only服务端403、数据库Hash不变、前端写按钮禁用且无控制台错误。唯一开发head为0104、默认关闭、不改旧`bid_intake_*`、不得应用到ECS；未使用真实PDF、OCR/视觉、Embedding/Reranker、生成模型或外部MCP。9004当前为完全隔离view-only。详见 `AI_Middle_Office/docs/bid-assessment-phase4c1-enterprise-capability-hard-gates-20260817.md`。

> 报价资料研判 Agent Phase 4B-5 v0.1-r50 已完成 Execute Preflight、前端 readiness、提交前进程级 Authority Fingerprint 再确认及 API-41/42/43 取消/Checkpoint重试操作面。Preflight 不返回密钥、绝对路径或正文；view-only 用禁用哨兵阻断父进程与`.env`真实Key，浏览器不能提升权限。自动矩阵`22 passed / 0 failed`，Python/PowerShell/JSON/Vite通过；动态 execute 仅创建1 Assessment且0 Run/Model/Tool，同库view-only四种写方法403且哈希不变。9003已升级为新版RQ2-B view-only，0 blocker、密钥围栏ready、5个历史Run可读，历史库SHA-256仍为`1EFC35CB...53942`；浏览器写按钮禁用且无错误。无新迁移，head保持0103；未运行PDF/OCR/视觉/Embedding/Reranker/生成模型/外部MCP，不连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-phase4b5-runtime-preflight-operations-20260817.md`。

> 报价资料研判 Agent Phase 4B-4 v0.1-r49 已完成隔离本地 Runtime Lab 双模式门禁：默认 view-only，后端硬阻断所有写方法且不启 Worker/模型；execute 必须显式开启，DeepSeek Key 在替换服务前 fail-closed。Capability 和前端共同校验 access/write/worker/model/retrieval readiness；启动/停止脚本具备隔离 Lab、自校验及 PID 缺失安全回退。自动合同/配置/SSE相邻矩阵 `8 passed / 0 failed`，Vite `2235 modules`；动态专项验证 execute Assessment POST 201但0 Run/0 ModelCall、view-only四种写方法403，临时库和Phase 4B-3历史库停启前后SHA-256一致。当前9003为view-only；不涉及PDF/OCR/视觉/模型/外部MCP，无新迁移，head保持0103。详见 `AI_Middle_Office/docs/bid-assessment-phase4b4-local-runtime-access-modes-20260817.md`。

> 报价资料研判 Agent Phase 4B-3 v0.1-r48 已完成本地真实资料可演示 MVP：在隔离 localhost/SQLite/本地对象目录控制平面中，以三份 Development 真实 PDF（383页）、固定本地 BCE exact-COSINE、RQ2-B 融合、Evidence MCP 与获授权联网的 DeepSeek V4 Flash 跑通全链；3/3 Run、3/3 Report、78/78 Task、3/3 Run Validation 成功，形成33 Fact、21 Claim、83 Atom引用，105次模型调用总成本约`$0.062962`。三份Parse仍因禁用OCR/视觉而为review_required；企业快照为合成数据，实时链未启用RQ2-C，正式Holdout未运行。当前可用于本地演示和简历，不能宣称生产上线；无新迁移，head保持0103，默认关闭，不得应用到ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4b3-real-pdf-resume-mvp-20260816.md`。

> 报价资料研判 Agent Phase 4B-2 v0.1-r47 已完成真实 DeepSeek V4 Flash + 本地 Evidence MCP 隔离合成 MVP-1 全链：26/26 Task、31/31 ModelCall、20/20 Tool、51/51 Run Validation Check 通过，Run succeeded、Report ready；失败响应Token/费用账本、Gateway Tool ID、UTC/CNY表示和检索提示参数边界已收口。总账`170165/5730` Token、`11430` micro-USD，直接相关专项`110 passed / 0 failed`。仅使用合成TXT，真实PDF生成质量与生产上线仍未完成；默认关闭，无新迁移，head保持0103，未使用OCR/视觉/外部MCP/生产Milvus或ECS。下一步为需另行授权的 Phase 4B-3 真实 Development PDF 生成质量评测。详见 `AI_Middle_Office/docs/bid-assessment-phase4b2-deepseek-isolated-mvp1-full-chain-20260816.md`。

> Phase 4B-1 v0.1-r46 的 Provider 最小真实复烟仍作为前置基线保留：合法 `finish`、输入/输出 `2240/42` Token、成本 `326` micro-USD，未包含业务资料。详见 `AI_Middle_Office/docs/bid-assessment-phase4b1-deepseek-v4-flash-provider-protocol-20260816.md`。

> 报价资料研判 Agent RQ2 总收口 v0.1-r45 已完成合同/Schema 与相邻专项 `126 passed / 0 failed`；三项目60题/156目标/19类别已全部批准为 Development Gold，Dataset Hash `50857d3f...c963`，Development Snapshot Hash `8ad34718...0a`。固定本地 BCE 正式跨项目 RQ2-B→RQ2-C A/B 的 Candidate Macro Hit@5/Recall@5 为 `0.966667/0.886667`，逐题退化0、Atom-only违规0、全不变量通过；但 Citable Target Availability `0.955855 < 0.98` 且 Paired Search Delta P95 `3705.179ms > 2500ms`，Development 准出失败。当前不得建立 Pre-Holdout Freeze 或运行 Holdout；先在 Development 修复6个不可引用目标与重排增量延迟，并以新 Candidate/Snapshot 再验。泰丰因旧 Agent Holdout暴露史只能作为Development。未调用OCR/视觉/生成模型/外部MCP/生产Milvus或外部环境；无新迁移，head保持0103。详见 `AI_Middle_Office/docs/bid-assessment-rq2-closeout-cross-project-gold-holdout-protocol-20260816.md`。

> 报价资料研判 Agent RQ2-C v0.1-r44 已完成代码、本地隔离专项与“香港中心”四臂消融：冻结 RQ2-B Top-20 Child，以固定 revision 本地 BCE Cross-Encoder 对 q1 原查询配对打分；保留 RQ2-B Top-K 骨架、Top-1和词法锚点，只允许分数至少0.30、分差至少0.08的最多2次尾部替换，零promotion必须逐项保持基线。Evidence MCP v7 Search不可引用、Read仍Atom-only，历史v4/v5/v6 Adapter冻结；无重复矩阵 `367 passed / 0 failed`。RQ2-B→RQ2-C Top-5 Hit/Recall保持 `0.96/0.90`，Top-8从 `0.96/0.90` 提升为 `1.00/0.94`，`HKC-C3-020`正确Child从Fusion rank11提升到最终rank7；4次promotion、21题恒等、逐题零退化、Atom-only违规0、重放一致。冻结Worker依赖下CPU P95 `1526→3584ms` 留作性能优化。无新迁移，head保持0103；未调用OCR/视觉/生成模型/外部MCP或生产Milvus，不连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-rq2c-lightweight-rerank-protocol-20260815.md`。

> 报价资料研判 Agent RQ2-B v0.1-r43 已完成代码、本地隔离专项与“香港中心”三组消融：同一 RQ1-C Query Plan 下融合 RQ1-D/RQ2-A Top-40 Child，以稳定 Key 去重，冻结词法1.00、语义0.35、重合奖励0.20、k=60的rank-only weighted RRF；真实资料修正 `child:`/`chunk:` Key兼容缺口。Evidence MCP v6 Search不可引用、Read仍Atom-only，历史v4/v5 Adapter冻结；无重复矩阵 `245 passed / 0 failed`。RQ1-D/RQ2-A/RQ2-B 的 Hit@5 `0.92/0.68/0.96`、Recall@5 `0.86/0.60/0.90`、Atom Read `0.82/0.60/0.86`、MRR `0.584/0.478/0.651333`；融合补回2题但1题Top-5/Top-8回退，下一步进入RQ2-C轻量重排。无新迁移，head保持0103；未执行生产Milvus、OCR/视觉、生成模型或外部MCP。详见 `AI_Middle_Office/docs/bid-assessment-rq2b-candidate-fusion-protocol-20260815.md`。

> 报价资料研判 Agent RQ2-A v0.1-r42 已完成代码与本地隔离专项验证：冻结 Child-only SemanticIndex/Entry/Head、BCE snapshot/768维 Normalize-COSINE、64 Child 分批 Heartbeat、Lease/Fencing、稳定 namespace/request id、Provider Hit Hash/去重门与 Evidence MCP v5 semantic-only Search；Read 仍 Atom-only。合同/迁移/状态机/历史 Adapter/Phase 3E/3F/API-41/SSE共 `190 passed / 0 failed`。“香港中心”共享权威25题真实固定 BCE exact-COSINE A/B 中，语义通道补回2个词法零命中题，但总体 Hit@5 `0.92→0.68`、Recall@5 `0.86→0.60`、Atom Read `0.82→0.60`且11题回退，确认 RQ2-A 只能作为候选源，下一步进入 RQ2-B 词法+语义候选融合，再进入 RQ2-C 轻量重排。线性 head 为 `20260815_0103`，默认开关关闭；本机未安全启动独立 Milvus daemon，A/B 明确使用非生产 `isolated-bce-exact-cosine`，生产 Milvus Adapter 真实联调保留为部署前门禁。未调用 OCR/视觉/生成模型/外部 MCP，未连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-rq2a-semantic-retrieval-protocol-20260815.md`。

> 报价资料研判 Agent RQ1-D v0.1-r41 已完成代码与本地隔离专项验证：在 C3 不可变索引上派生五通道字段投影，以稳定 evidence key 生成 Hash/tie-break；保留 RQ1-C Child BM25基线，以弱字段0.005/表格强结构0.10作有界 BM25F tie-breaker，并加入模板DF抑制、q1 Anchor、Parent 0.20与多样性。Evidence MCP v4/历史 Adapter兼容，合同、C3、Phase 3E/3F/API-41/SSE及迁移拓扑共 `205 passed / 0 failed`。“香港中心”共享权威25题 A/B 中 Hit@5 `0.68→0.92`、Recall@5 `0.62→0.86`、Atom Read `0.62→0.82`、零命中 `8→2`，6题提升且逐题零回退；P95 `1195→1708ms`。用户已决定暂缓预分词/语料缓存性能优化，路线直接进入 RQ2-A 语义索引与召回。无新迁移，head保持 `20260814_0102`；未调用 OCR、视觉、模型、Embedding、向量服务或外部 MCP。详见 `AI_Middle_Office/docs/bid-assessment-rq1d-field-aware-lexical-protocol-20260815.md`。

> 报价资料研判 Agent RQ1-C v0.1-r40 已完成代码与本地隔离专项验证：Query Plan v2 固定原查询锚点，按并列主体、通用投标字段别名和答案形状扩展并执行旧 Planner atomic/fact-slot；最多6条、稳定去重、Child BM25 + Parent 0.35 辅助 weighted RRF。新 `evidence.search@v3-rq1c` Adapter 与历史 v2 Dispatch 隔离，复用现有 C3 Index；合同/配置/Evidence MCP/Adapter/Phase 3E/3F/API-41/SSE/迁移拓扑无重复矩阵 `202 passed / 0 failed`。“香港中心”25题共享 ParseHead/IndexHead A/B 中平均查询 `1.08→3.84`、Hit@5 `0.44→0.72`、Recall@5/Atom Read Recall@5 `0.38→0.66`、Hit@8 `0.48→0.84`、零命中题 `14→7`，证据角色与确定性无回退；Top-5 指标仍未到建议门槛且 Search P95 `397→1065ms`，下一步进入 RQ1-D 字段感知词法召回与通道消融。无新迁移，head 保持 `20260814_0102`；未调用 OCR、视觉、模型、向量服务或外部 MCP。详见 `AI_Middle_Office/docs/bid-assessment-rq1c-deterministic-query-optimizer-protocol-20260815.md`。

> 报价资料研判 Agent RQ1-B v0.1-r39 已完成代码与本地隔离专项验证：新增独立默认关闭的 v4 Parser Profile 和 `bid.parse.quality.v1`，以 Native readiness 30、Structural coherence 35、Citable integrity 25、Warning hygiene 10 四维确定性评分生成 pass/review_required/blocked；partial 最高84分，硬阻断最高39分。报告进入 Parse result hash，Worker 校验唯一性/血缘/状态/Hash；blocked 阻断 C3 Index、Lot Detection 和 Phase 3 Run Bootstrap。旧 v1/PDF-C2 v2/RQ1-A v3 Profile 与 Retrieval input hash 不变；复用既有表，不新增迁移，head 保持 `20260814_0102`。合同/Schema/配置、Parser/Worker/C3、Phase 3A/API/Evidence MCP 与迁移拓扑无重复矩阵 `179 passed / 0 failed`；“香港中心”25题 A/B 将 `partial + high/100` 修正为 `partial + medium/84 / review_required`，结构与检索指标不变，Hit@5/Recall@5/Atom Read Recall@5 仍为 `0.44/0.38/0.38`。下一步进入 RQ1-C Query Optimizer；未调用 OCR、视觉、模型、向量服务或外部 MCP。详见 `AI_Middle_Office/docs/bid-assessment-rq1b-parse-quality-gate-protocol-20260815.md`。

> 报价资料研判 Agent RQ1-A v0.1-r38 已完成代码与本地隔离专项验证：新增独立默认关闭的 `bid-document-parser-profile-v3-pdf-structure-rq1a`，按跨页重复+顶部/底部 10% 几何区+通用数字折叠抑制页眉页脚；按顶层章节聚合微型小节、聚合连续矢量表格行，并把每个 heading 原文保留为可引用 Atom。旧 PDF-C2 v2/C1 v1 Profile 不变；C3 仍按 ParseHead/Hash fail-closed。合同/配置/降噪/Hash/聚合/证据门/兼容及 Phase 2/C3/Evidence MCP 相邻回归无重复矩阵 `48 passed / 0 failed`。“香港中心”25 题 A/B 中 Warning `1432→588`、可引用目标 `88.10%→100%`、Hit@5 `0.32→0.44`、Recall@5 `0.24→0.38`、Atom Read Recall@5 `0.16→0.38`，逐题无上述指标回退；但仍低于建议门槛，且质量投影仍错误显示 `high / 100`。下一步进入 RQ1-B Parse Quality Gate，之后进入 RQ1-C Query Optimizer；复用既有表，不新增迁移，head 保持 `20260814_0102`。未调用 OCR/视觉/模型或外部服务；详见 `AI_Middle_Office/docs/bid-assessment-rq1a-structure-aggregation-protocol-20260815.md`。

> 报价资料研判 Agent PDF-C2/C3 真实资料 Silver 基线 v0.1-r37 已完成：经用户授权，在完全隔离的本地 SQLite 环境对 1 份 307 页真实 PDF 运行原生布局解析、Parent/Child/Atom、C3 Index 和 Evidence MCP v2，并建立 25 题/42 目标的单审阅者 Silver 集。结果为 `Hit@5=0.32`、`Target Recall@5=0.24`、`MRR@5=0.1667`、`Atom Read Target Recall@5=0.16`；5/42 目标无可引用 Atom，88.01% Child 低于 220 token，1432 条结构警告仍被错误投影为 `high / 100`。证据角色门保持 0 违规且确定性重放一致，但真实检索质量明确未达可用门槛。下一步进入 Retrieval Quality-1：先修结构/可引用性与质量分，再做确定性 Query Optimizer、字段感知词法召回，最后接语义召回/重排；Silver 需业务复核为 Gold 后才能形成正式准确率结论。未调用 OCR、视觉、模型、向量服务或外部 MCP，未连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-pdf-c3-real-document-silver-baseline-20260815.md`。

> 报价资料研判 Agent PDF-C3 v0.1-r36 已完成代码增量与本地隔离专项验证：冻结 `bid.evidence.retrieval-index.v1` / `bid-evidence-retrieval-profile-v2-role-aware` / Evidence MCP v2，Search 只返回不可引用 Child、Parent 只做 0.35 权重辅助召回、Read 只返回同 Parent 可引用 Atom；新增 RetrievalIndex/Entry/Head 权威、ParseHead/Profile/Manifest/Hash 失效门、历史 v1 Adapter 围栏、默认关闭配置和线性 `20260814_0102`。PDF-C3/合同/配置/0102 迁移主矩阵 `165 passed`，PDF-C2/C1/Phase 2 相邻回归 `28 passed`，Phase 3E/3F/MVP-1/API-41/SSE 相邻回归 `18 passed`，授权矩阵共 `211 passed / 0 failed`；全部仅使用合成结构数据和隔离 SQLite/Alembic 环境，未读取真实 PDF，未调用 OCR/视觉/模型/向量服务或外部 MCP。下一步应先建立检索质量评测基线，再据真实缺口决定进入 PDF-C4（OCR/混合页接入）或 Retrieval Quality-1（语义召回/重排）。详见 `AI_Middle_Office/docs/bid-assessment-pdf-c3-role-aware-retrieval-protocol-20260814.md`。

> 报价资料研判 Agent PDF-C2 v0.1-r35 已完成代码增量与本地隔离专项验证：新增默认关闭的原生布局 Parser Profile，以 pdfplumber 原生字符/Word/矢量表格层生成 page/bbox/reading order/section path 结构块，经唯一 PDF-C1 Builder 映射为 Phase 2 Parent/Child/Atom；结构判断不使用业务关键词，空白/扫描/低覆盖页只标记待 OCR，不调用 OCR/视觉/模型。旧 v1 Parser Profile 保持兼容，v2 需总运行时、Phase 2 Worker、PDF-C2 子开关和 Profile 四项一致才能启用；复用现有表，不新增迁移，head 保持 `20260813_0101`。合成布局/配置/映射/Lot Atom 门禁、机器合同及 Phase 2 Parse/Lot Worker 相邻回归合计 `91 passed / 0 failed`；只生成并解析内存合成 PDF，未读取或渲染真实 PDF，未调用 OCR/视觉/模型/MCP。下一步进入 PDF-C3：冻结 role-aware Evidence MCP/Retrieval Profile、Index Head 与失效规则，再判断是否需要线性 `0102`。详见 `AI_Middle_Office/docs/bid-assessment-pdf-c2-native-layout-protocol-20260814.md`。

> 报价资料研判 Agent PDF-C1 v0.1-r34 已完成代码增量与本地隔离专项验证：冻结 `bid.evidence.chunk.v2` 和 220/380/500/600 Token Profile，实现 Section Parent→Retrieval Child→Evidence Atom、结构硬边界、正文动态聚合、仅超长块 80 Token overlap、确定性 context prefix/retrieval text 与稳定 Hash；只有 Atom 可引用。合同/Token/边界/overlap/Parent-Child-Atom/Span 覆盖/稳定 Hash 专项 `12 passed`，Phase 2 Parse Worker 结果合同相邻回归 `4 passed`。PDF-C1 本身仍是不读取 PDF 的纯 Builder，现由默认关闭的 PDF-C2 v2 Profile 单向调用；不调用 OCR/视觉/模型，不新增迁移，head 保持 `20260813_0101`。详见 `AI_Middle_Office/docs/bid-assessment-pdf-c1-structure-chunk-protocol-20260814.md`。

> 报价资料研判 Agent 可运行 MVP-1 v0.1-r33 已完成完全隔离的本地落地验证：新增 localhost-only 服务、独立 SQLite/对象目录、进程内 Outbox/Worker、注入式确定性 Provider、合成样例和 HTTP 验证器；Assessment→上传→Manifest→原生文本解析→标段→P0—P4→Evidence MCP→Fact/Gate/Decision/Claim/Report→Run Validation 全链实际收敛为 `Run=succeeded`、`Report=ready`。合同/迁移/Planner/LangGraph/配置及 API/Worker/事务/恢复共 `310 passed / 0 failed`。代码唯一 Alembic head 保持 `20260813_0101`，正式开关默认关闭；9001 仅为隔离实验环境，真实资料、OCR/视觉、真实模型/外部 Tool 和 ECS 上线仍未授权。详见 `AI_Middle_Office/docs/bid-assessment-phase4-mvp1-vertical-slice-protocol-20260813.md`。

> 报价资料研判 Agent Phase 4A-2 v0.1-r30 已完成代码增量与本地隔离专项验证：新增受控 Model Gateway、三张模型执行权威表、Provider Attempt Lease/Heartbeat/Fencing/稳定 request id/幂等与未知结果恢复、成本和 Token 预算、总超时及未领取调用恢复；单 Task LangGraph 每次只生成一个受约束动作，无自身 Checkpointer，模型/Tool/候选均回到 Phase 3 控制面持久化，Run Validator 升级为 v4。合同/0099/有界执行、Phase 4A-1 与 Phase 3C—3G/API-41、SSE/Outbox/事务/幂等共 `189 passed / 0 failed`。新增默认关闭 `FEATURE_BID_ASSESSMENT_PHASE4_LOCAL_AGENT/MODEL_EXECUTOR` 和线性 Alembic `20260813_0099`；仅使用本地 SQLite 和显式注入测试 Provider，未注册或调用真实 Provider、MCP、OCR/视觉、外部 Tool、真实样例或真实存储，未连接外部环境，不修改旧 `bid_intake_*`，不得应用到 ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4a2-model-langgraph-executor-protocol-20260813.md`。

> 报价资料研判 Agent Phase 4A-1 v0.1-r28 已完成代码增量与本地隔离专项验证：同一 preliminary/reanalysis Run 现在可按确定性 P0—P4 模板续段，每段仍最多 8 个新 Task、局部深度最多 3；阶段完成写 `bid.plan.continuation_requested.v1`，旧 current Plan 在新 Revision 同事务提交时转为 superseded，只有 P4 完成才进入 Run Validation。新增仓库版本化 8 个 Skill artifact，Plan v2 envelope/Task input hash 冻结 catalog 与 `skill_id/version/hash/executor_kind/action_contract/output_schema/allowed_tools`，历史 TaskContract 按 retained artifact fail-closed 重构；Run Validator v3 校验全部 Revision 联合血缘。合同/Planner/Continuation 事务恢复/历史 TaskContract/API-41/SSE/0083—0098 迁移拓扑共 `173 passed / 0 failed`。新增默认关闭 Phase 4 总/子开关与线性 Alembic `20260812_0098`；未调用模型/MCP/OCR/外部 Tool、真实样例或真实存储，未连接外部环境，不修改旧 `bid_intake_*`，不得应用到 ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4a1-plan-continuation-skill-binding-protocol-20260812.md`。

> 报价资料研判 Agent Phase 4 可落地执行架构 v0.1-r26 已冻结：Phase 3 继续作为唯一 Run/Plan/Task/Attempt/Lease/Fence/Checkpoint/Context/Tool/Validation 控制平面，LangGraph 只作为单 Task 内的一步一检查点状态转换器，MCP 固定为 Tool Gateway 后的只读 Adapter；旧 Agent 复用 Query Planner、exact/semantic/hybrid 路由、BM25 + BCEmbedding/Milvus + RRF、MCP transport、证据门和评测方法，不复用旧 `bid_intake_*` Repository、Checkpoint 或单体 ReAct 运行权威。Phase 4A-1 Plan Continuation + SkillBinding 与 Phase 4A-2 受控 Model Gateway + LangGraph Executor 均已验证完成；随后按新数据域 Evidence MCP/检索 -> 十类事实权威 -> HG01—HG07/确定性 Decision/Claim/初筛报告 MVP-1 推进，仍不得应用到 ECS。详见 `AI_Middle_Office/docs/bid-assessment-phase4-landable-agent-architecture-20260812.md`。

> 报价资料研判 Agent Phase 3 总收口 v0.1-r25 协议、代码与本地隔离综合验证已完成：已冻结机器可读完整运行 Profile，新增默认关闭总开关并强制 V1 Runtime 与 Phase 3A—3G 依赖闭包；Run Validator 升级为 `bid-run-integrity-validator-v2`，把 Task Attempt/Checkpoint、Context Manifest、Invocation、AsyncOperation、Dispatch/DispatchAttempt、ToolResult 的 Hash/Fence 血缘纳入确定性 input 与终态校验；API-40→Plan Commit→Task/Context/本地只读 Adapter→新 Attempt/Fence→final Checkpoint→Validation→API-41/SSE 确定性端到端及综合矩阵共 `175 passed / 0 failed`。不新增迁移，代码唯一 head 保持 `20260812_0097`；真实模型、OCR/视觉、公网、真实外部工具和对象存储继续关闭，未连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3-closeout-protocol-20260812.md`。

> 报价资料研判 Agent 新数据域 Phase 3A 已完成 v0.1-r17 协议、Run Bootstrap 代码与本地隔离专项验证：复用 Phase 2 当前 Scope/Manifest 和既有 `0084`—`0086` 运行骨架，从 `bid.plan.requested.v1` 原子选择最新合法 frozen 企业快照与六类唯一 active 配置，以数据库 UTC 时间固化 `input_fingerprint/input_hash`，创建 Run、更新 Assessment active 指针并写 `bid.run.created.v1`、审计和 processed marker；输入未就绪时不创建占位 Run且不写 marker，由维护扫描恢复。API-40/API-41 已实现手动 reanalysis、幂等/ETag/ACL 与无内部 Task DAG 的进度投影。本增量不执行 Planner、模型、Tool 或 OCR，不新增 migration，代码唯一 head 保持 `20260811_0093`；四个相关运行开关默认 false，旧 `bid_intake_*` 未修改。获准专项共 `82 passed`（合同 64、Phase 3A 核心 3、API-03/API-31/API-32 相邻 8、事务/Outbox/SSE 运行服务 7）；未运行迁移拓扑、真实样例、OCR/视觉解析或模型调用。未连接或改动 ECS/CentOS/真实 MinIO/Redis；目标 ECS 最近只读确认仍为 `20260808_0082`。Phase 3B 当前状态见下一条，详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3a-protocol-20260811.md`。
> 报价资料研判 Agent 新数据域 Phase 3B 已完成 v0.1-r18 协议、代码与本地隔离专项验证：新增 49 项标准任务运行时注册表、无模型初始 PlanProposal、九项确定性 DAG 校验、可复现 Plan envelope、`bid.run.created.v1` 幂等消费、PlanRevision/8 Task/7 Dependency 原子提交、`bid.plan.committed.v1` / 首个 `bid.task.ready.v1`、审计和维护扫描恢复。首批 DAG 最大动态深度 3，只复用 Phase 2 权威 Manifest/ParseHead/Scope，不从文件名、MIME 或 parser_hint 重新推断标段；不创建 Attempt、不调用 OCR/视觉/模型。独立开关 `FEATURE_BID_ASSESSMENT_PHASE3_PLANNER=false` 默认关闭；复用 `0085/0086`，不新增 migration，代码 head 仍为 `20260811_0093`，旧 `bid_intake_*` 未修改。获准专项共 `133 passed`（合同 65、Planner/DAG 5、Plan Commit 与 API-40/API-41 相邻 4、迁移拓扑 47、事务回滚/维护恢复 2、Outbox/processed marker/SSE 运行服务 10）；未运行真实样例、OCR/视觉解析、评测或模型调用，未连接或改动外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3b-planner-protocol-20260811.md`。
> 报价资料研判 Agent 新数据域 Phase 3C 已完成 v0.1-r19 协议、Task Runtime Control Plane 代码增量和本地隔离专项验证：冻结 TaskContract 权威重构、Task/Attempt Lease、180 秒租约、心跳/fencing CAS、不可变 Checkpoint、完成回执、依赖释放、validation request、失败重试、租约恢复和 terminal Run fence；独立开关 `FEATURE_BID_ASSESSMENT_PHASE3_TASK_RUNTIME=false` 默认关闭，维护任务不主动领取 ready Task。本阶段复用 `0085/0086`，无需新 migration，代码 head 保持 `20260811_0093`；不执行模型、OCR、视觉、Tool 或真实对象存储，不新增外部 API，不修改旧 `bid_intake_*`。Phase 3C 合同、状态机、API-41/SSE 相邻回归与迁移拓扑共 `123 passed`，未连接或改动外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3c-task-runtime-protocol-20260811.md`。
> 报价资料研判 Agent 新数据域 Phase 3D 已完成 v0.1-r21 协议、代码增量和本地隔离专项验证：API-42 持久化取消意图并由周期维护原子围栏 Task/Attempt/AsyncOperation、收敛 Run/Assessment；API-43 在原 failed/retryable Run 下执行当前 Scope/Manifest/active Run stale 门禁，创建递增 Attempt/Fencing 并由下一次 Lease 复用，同时携带最近 Checkpoint 恢复引用。API-41 已纳入 `cancel_requested_at` 与动态 allowed actions，Outbox/SSE 投影补齐取消、已取消和重试事件；独立开关 `FEATURE_BID_ASSESSMENT_PHASE3_RUN_LIFECYCLE=false` 默认关闭。Phase 3D 复用 `0085/0086` 已有结构，并新增 `20260811_0094` 仅扩展 `bid.run.retry_requested.v1` 的数据库 Outbox CHECK，代码唯一 head 为 `20260811_0094`。授权范围内机器合同、迁移、API/状态机相邻链和运行服务共 `137 passed`；未执行模型、OCR、视觉、Tool、真实样例或真实对象存储，不修改旧 `bid_intake_*`，未连接或改动外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3d-run-lifecycle-protocol-20260811.md`。
> 报价资料研判 Agent 新数据域 Phase 3E 已完成 v0.1-r22 协议、Tool/Context Control Plane 代码增量与本地隔离专项验证：确定性 Context Manifest、服务端 Scope/冻结版本注入、严格 Tool Schema/profile/预算/幂等/HMAC scope token、不可变 Result Store、同步/异步结果与 Checkpoint/新 Attempt-Fence 恢复已经落地；取消和显式重试同步围栏未完成 Invocation。新增默认关闭开关 `FEATURE_BID_ASSESSMENT_PHASE3_TOOL_CONTEXT=false` 和线性 revision `20260812_0095`，代码唯一 head 为 `20260812_0095`。授权范围内合同与迁移拓扑 117、API/Phase 3C/3D 相邻链 16、Outbox/SSE/维护恢复 13，共 `146 passed`；本阶段不开放执行器或新外部 API，未执行模型、OCR、视觉、Tool、真实样例或真实对象存储，未连接或改动外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3e-tool-context-protocol-20260812.md`。
> 报价资料研判 Agent 新数据域 Phase 3F 已完成 v0.1-r23 协议、受控 Tool Adapter/Executor 代码增量与本地隔离专项验证：已落地原子 Dispatch 意图、DispatchAttempt Lease/Fence、稳定 provider request id、安全幂等重放、发送后未知结果与取消/超时联动。首个 Adapter 仅为本地只读 `documents.outline`，不读取原文件、不触发解析、不调用模型或外部工具。新增默认关闭开关 `FEATURE_BID_ASSESSMENT_PHASE3_TOOL_EXECUTOR=false` 和线性 revision `20260812_0096`，代码唯一 head 为 `20260812_0096`；授权范围内合同、迁移、执行/恢复及相邻回归共 `149 passed`，不开放新外部 API，不连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3f-tool-executor-protocol-20260812.md`。
> 报价资料研判 Agent 新数据域 Phase 3G 已完成 v0.1-r24 协议、Run Validation/Convergence 代码与本地隔离专项验证：已落地每 Run 唯一 Validation、ValidationAttempt Lease/Fence、确定性 frozen input/Plan/Task/Dependency/Attempt/Checkpoint/AsyncOperation/Tool 血缘校验、不可变 result hash，以及 Run/Assessment/Outbox/Audit 原子 succeeded/failed/stale 收敛。新增默认关闭开关 `FEATURE_BID_ASSESSMENT_PHASE3_RUN_VALIDATION=false` 和线性 revision `20260812_0097`，代码唯一 head 为 `20260812_0097`；核心与 Phase 3C—3F/API-41/SSE 相邻回归共 `158 passed / 0 failed`，不调用模型、OCR/视觉、外部工具或真实对象存储，不连接外部环境。详见 `AI_Middle_Office/docs/bid-assessment-runtime-brain-phase3g-validation-convergence-protocol-20260812.md`。
...
> 执行系统已按产品决定退役（2026-07-31）：执行任务、会议纪要和执行速度看板的前端、API、服务、模型、RBAC 入口与功能开关均已移除；Alembic `20260731_0078` 删除 5 张专用表，历史迁移保留。`project_*` 项目进度与项目任务不属于该系统，继续保留。数据库升级前必须先备份。

> 报价审阅字段、定额行交互与统计导出已完成当前环境验收（2026-07-31）：快速审核不再展示“施工提示”；专业全字段固定为用户确认的 14 列。展开后的定额主项只保留无表头数据行，以“定额编码”替换“序号”，其余字段与外层项目列逐项像素对齐；重复标题、来源标签、分级路径、具体匹配标题和行内工料机按钮均已移除。统计主材/辅材去掉项目特征及工作内容，并新增四 Sheet Excel 导出。不新增 Alembic，不改变报价价格口径、项目定额编辑规则或企业定额 active 数据；联合回归 `72 passed`，Vite production build 通过。详见 `AI_Middle_Office/docs/project-quota-resource-workbench-20260731.md`。

> 项目定额与工料机工作台第一版已完成独立模块交互验收（2026-07-31）：原右侧抽屉改为“报价草稿”和“导入甲方清单”之间的常驻“工料机明细”模块；点击专业全字段中定额数据行任意位置只加载当前定额工料机并保持页面当前滚动位置，继续支持全部业务字段新增、编辑、删除。变更会重算项目定额并同步当前项目计价草稿，不影响企业定额 active；只有显式选择同步且具备 `cost_approver` / 管理员权限时才写入企业定额 draft。Alembic 仍为 `20260731_0076`，不新增迁移。真实项目 23 的 `ZS00023` 已成功加载 1 条工料机，模块顺序、整行点击、选中态、页面不跳转和抽屉移除均通过浏览器验收。详见 `AI_Middle_Office/docs/project-quota-resource-workbench-20260731.md`。

> 对话报价摘要到预算项目详细工作台的同草稿数据闭环已完成（2026-07-29）：新增 `POST /api/v1/quote/jobs/{job_id}/budget-workspace` 与 Alembic `20260729_0073`，跳转前保存当前预审，按原 Excel SHA-256 复用预算项目或自动建立项目/正式导入，并把项目、工程量、单位、当前确认价、账户/企业/AI 来源、费用拆分和工艺备注同步到现有计价草稿；任务持久化项目/草稿关联与内容哈希，相同内容重复进入不重建。28 行联昇真实联调复用项目 21 / 草稿 12，两端总额 `¥1,133,009.73`，来源分布 `1 / 0 / 27`，详细页专业字段和工艺备注已完成浏览器验收。当前数据库已到 head；9000 高权限旧进程需管理员重启后加载新路由。详见 `AI_Middle_Office/docs/unified-quotation-chat-entry-correction-20260728.md`。

> 报价资料研判 Agent 自适应 Tool 预算已完成代码层验证（2026-07-28）：ReAct 改为“首轮 1 次主检索 → 检索后最多读取 2 条关键证据 → 上下文读取后最多 1 次定向补查”，不再让模型持续吃满每轮 3 个 Tool 的静态上限；Runtime 按阶段优先级确定性裁剪冗余请求，图谱记录阶段、上限、原因、原始请求数、实际保留数和裁剪数。总调用预算、重复参数预算、Tool 白名单、证据门和人工审核保持不变。联合回归 `67 passed`，Query Planner 回归 `9 passed`，compileall 与 diff check 通过；不新增 Alembic。详见 `AI_Middle_Office/docs/bid-intake-agent-adaptive-tool-budget.md`。

> 报价资料研判 Agent 自适应检索路由已完成代码层与当前环境运行态验证（2026-07-28）：Query Planner 的每个原子 Query 由确定性轻量分类器选择 `exact / semantic / hybrid`，事实与关键词查找走直接标识匹配 + BM25，风险/影响/建议等研判问题走向量，只有精确标识与语义研判并存时直接走向量 + BM25 + RRF；单通道零结果可升级 hybrid 兜底。分类器不调用 LLM，MCP Observation 与运行图谱保留分类、置信度、原因、实际通道和兜底审计；Tool 参数、数据库及 Milvus schema 不变。Agent/Tender 聚焦回归 `70 passed`；CentOS 通道验收确认 exact 不生成查询向量、semantic 不执行 BM25、hybrid 执行双路 RRF；东莞香港中心 1869 块真实索引已分别验证 `1 semantic + 3 exact` 与 `1 hybrid + 2 exact`，均返回 5 条证据；MCP/Worker 重启后 readiness `ready_to_start=true`。详见 `AI_Middle_Office/docs/bid-intake-agent-adaptive-retrieval-router.md`。

> 旧对话报价流式进度已完成普通用户业务化（2026-07-28）：不再把网关、模块、队列、任务号、追踪号等后台原始事件写入聊天气泡，改为“接收需求、读取内容、整理清单、计算报价、准备核对”五阶段业务进度卡；新增按真实阶段受控并持续变化的进度条、友好耗时和非技术性失败建议。联合回归 `45 passed`、旧页脚本解析和 Vite build 通过；浏览器不下发验收报价观察到计算阶段 `77%`、进度宽度 `76.8%`、聊天正文无技术信息，并正常进入核对窗口。无 Alembic、无报价规则或定额数据变更。详见 `AI_Middle_Office/docs/unified-quotation-chat-entry-correction-20260728.md`。

> 统一报价工作台第一版界面融合已完成（2026-07-28）：预算项目计价页与旧对话预审弹窗统一为“快速审核、专业全字段、费用汇总、报价依据、版本记录”五视图，并共用原有草稿；专业视图保留完整项目、工程量、价格和费用拆分字段，原基础定额/账户定额对比收纳为高级计价策略，风险复核、证据、草稿、导出和下发均保留。无 Alembic、无报价规则或定额数据变更；联合回归 `44 passed`、Vite build 和旧页脚本解析通过，预算项目 20 浏览器验收确认五视图与 `198/198` 工程量。调试阶段继续保留两个入口；旧对话 299 行大预审自动化浏览器响应超时，需补业务实际弹窗走读。详见 `AI_Middle_Office/docs/unified-quotation-chat-entry-correction-20260728.md`。

> 统一报价当前进入双入口调试阶段（2026-07-28）：保留旧对话报价 `/index.html?entry=new-quote&mode=quick`，同时恢复“项目报价” `/quotes` 与 `/quotes/new`；待两条流程全部调通并验收后再隐藏其中一个入口。对话报价支持文字、图片、`.xlsx/.xlsm` 自动标准化，并按“当前账户 active 定额 → 严格唯一 active 企业定额 → 仅未命中行 AI 估价”逐行报价，继续复用原预审、草稿恢复、完整性检查和确认下发；项目报价聚合任务、历史和有权限可见的预算项目。三级报价与相邻回归 `115 passed`，双入口聚焦测试 `38 passed`，报价任务/历史/预审草稿/标准化/预算项目联合回归 `134 passed`，Vite build 和当前 9000 双入口路由/健康检查通过。详见 `AI_Middle_Office/docs/unified-quotation-chat-entry-correction-20260728.md`。

> 报价资料研判 Agent Phase 5d-1 细粒度执行链与紧凑图谱已完成代码层和前端视觉验证（2026-07-28）：trace 新增 LLM 输入、行动计划和循环判断节点，真实展示“向 LLM 提交什么安全上下文、选择什么工具、Tool 如何返回 Observation、Observation 如何回写、下一轮 ReAct 是否继续”；D3 节点核心半径从 26px 缩至 18px，自动适配不再放大少量节点，并增加带语义的连线和右侧详情。模型私有思维链仍不记录，改用可审计决策摘要。聚焦回归 `19 passed`、Vite build 通过、Chrome 紧凑布局验收通过；运行态需重启后新发起研判。

> 报价资料研判 Agent Phase 5d 实时运行图谱已完成代码层与前端视觉验证（2026-07-28）：LangGraph 执行节点已投影为可持久化的安全 trace 事件，覆盖 ReAct、工具授权、Tool、Observation、结构化草稿、总经办标准、证据门和 Human-in-the-loop；`/admin/bid-intake-agent` 新增 Vue 3 + D3 力导向运行图谱、节点详情、自动跟随、缩放拖拽与旧记录生命周期降级回放。只显示可审计摘要，不显示模型私有思维链，敏感字段脱敏；复用现有 `bid_intake_run_events`，不新增 Alembic。聚焦回归 `18 passed`、Vite build 通过、Chrome 已确认旧记录图谱可见；完整 ReAct/Tool/Observation 动态图需重启 FastAPI 与 Agent Worker 后新发起研判验收。详见 `AI_Middle_Office/docs/bid-intake-agent-phase5d-live-runtime-graph.md`。

> 报价资料研判 Agent Phase 5c 招标资料类型自动识别已完成代码层验证（2026-07-28）：Agent 页面已删除“本批资料类型”人工下拉框，上传统一进入自动识别；证据解析管线在文本抽取后联合文件名和正文结构，识别招标文件、答疑/澄清、补遗/变更、图纸、工程量清单或其他资料，分类结果进入 parsed file、evidence document 与任务审计事件。低置信度资料安全归为 `other`，不阻断证据入库；不新增 Alembic，不改变 ReAct、总经办标准或报价链路。联合聚焦回归 `32 passed`，compileall、Vite build 和 diff check 通过；运行态生效需重启 FastAPI 与 Celery Worker，详见 `AI_Middle_Office/docs/bid-intake-agent-phase5c-auto-file-type-classification.md`。

> 报价资料研判 Agent Phase 5b 前端资料入库闭环已完成代码层与当前运行态验证（2026-07-28）：`/admin/bid-intake-agent` 已直接提供“上传并解析招标资料”，支持 PDF、DOCX、XLSX、XLSM、TXT、MD 多文件入队、资料分类、解析进度、失败原因和可恢复任务重试；后台成功解析后自动生成新的 active evidence manifest，并自动刷新 Agent readiness，资料未就绪时继续保留确定性证据门。复用 Phase 3a/3b 与 Phase 5a 底座，不新增 Alembic，不改变总经办标准或报价链路；Vite build 通过，聚焦回归 `25 passed`，`/health/ready=ready`、Celery Worker 2 个在线，运行中的 SPA 已引用最新构建资源。待前端业务人工验收，详见 `AI_Middle_Office/docs/bid-intake-agent-phase5b-evidence-upload-workbench.md`。

> 报价资料研判 Agent Phase 5a 运行态启用与首个闭环已完成当前环境验收（2026-07-27）：当前环境已启用独立 LangGraph Worker 与数据库模式 Tender Evidence MCP，复用现有 DeepSeek OpenAI-compatible 路由；受控项目已跑通证据 manifest、ReAct Tool Calling、`AssessmentDraft` JSON Schema、PolicyEngine、确定性证据门、Human-in-the-loop 与 SQL Checkpoint 恢复，最终保留 assessment `61b98bd2-fbbe-474a-8514-fa08e6b5d25c`，人工“补资料”命令已 applied，run `completed / waiting_supplement`。Agent/Runtime/MCP/Evidence 聚焦回归 `50 passed`；9000 FastAPI 已由用户完成重启。随后建议部署 CentOS 招标证据混合索引，并用 5–10 个历史项目做质量与政策一致性校准。
...
> 原 Phase 3 执行任务/执行速度与 Phase 4a 会议纪要曾于 2026-05-19 完成验证，已于 2026-07-31 按产品决定退役；当前产品不再提供这些入口或接口。
...
> 原 BIZ-1a 商务台账已于 2026-07-31 按产品决定完整退役：移除功能入口、API、服务、模型、权限和开关；Alembic `20260731_0079` 删除 outbound 台账记录、`client_inquiry_events` 与台账专用字段，保留 Phase 2 inbound 客户咨询和响应速度追踪。
>
> 原“成本测算闭环”（COST-MEASURE-1 / COST-MEASURE-2）已于 2026-07-31 按产品决定完整退役：只移除 `cost_measurement*` 功能链；Alembic `20260731_0080` 删除三张专属表。项目/对话报价、成本库、企业/账户定额、智能组价、预算项目计价、报价资料研判 Agent 和智能助手继续保留。
...
> 商务/市场部提效路线已退役（2026-07-31）：BIZ-1a 台账及后续传统 CRM 式延展不再属于当前产品范围；早期设计与验收记录仅作历史追溯。
...
- **数据库规范**：当前代码与内网数据库迁移 head 均为 `20260801_0081`；`0078`—`0080` 已分别退役执行系统、商务台账和成本测算闭环的专用结构，`0081` 只新增 `budget_project_pricing_run_draft_snapshots`。0081 前全量备份为 `AI_Middle_Office/backups/pre_0081_pricing_snapshot_recovery_20260801_092701.sql`（SHA-256 `64DC1AA14C68DBB474B1C02031C051551FFE1EB4991CB88AFD3CCADF891580A3`）；当前内网环境已经在 head，不得重复执行迁移，其他环境执行任何迁移前仍必须备份。报价反馈、Prompt 回归、知识候选、Phase 0 RBAC、Phase 2 inbound 响应速度、成本库、企业/账户定额、项目进度、预算项目与 Agent 数据结构继续保留。后续新增字段或表必须新增 Alembic revision，不能退回依赖 `AUTO_CREATE_TABLES` / 启动兼容迁移。
...
- **原 Phase 3 / Phase 4a 执行系统已退役**：执行任务、执行速度与会议纪要不再属于当前产品范围；保留本路线图中的早期验收记录仅用于历史追溯。
...
- **原 BIZ-1a 商务台账已退役**：2026-07-31 已移除入口、API/service/schema、事件模型、RBAC 模块和开关；`20260731_0079` 清理其数据与专用 schema。Phase 2 客户咨询继续保留，不再需要 direction 隔离。
- **原成本测算闭环已退役**：2026-07-31 已移除 `/admin/cost-measurement`、`/api/v1/admin/cost-measurements/*`、service/model、RBAC 模块和开关；`20260731_0080` 只删除 `cost_measurements`、`cost_measurement_lines`、`cost_measurement_events`。其他成本、报价、定额、预算计价和 Agent 模块继续保留。
...
- **商务/市场部路线已退役**：BIZ-1a、BIZ-1b、BIZ-1c 和传统 CRM 式延展不再进入当前产品路线；如未来重新立项，需另行设计独立数据模型。
...
- **client_inquiries 恢复纯 inbound 模型**：`20260731_0079` 删除商务台账 outbound 数据和 `direction` 等专用字段；Phase 2 `FEATURE_CLIENT_INQUIRY` 与 `FEATURE_DASHBOARD_RESPONSE` 的客户咨询、报价关联和响应速度口径继续保留。
...
| 编号 | 名称 | 对应部门 | 优先级 | 依赖 |
|------|------|---------|--------|------|
| BIZ-1 | 商务/市场获客情报 | 商务部 / 市场部 | 已退役（2026-07-31） | 历史记录仅作追溯 |
...
**退役结论**：BIZ-1a 对核心效率提升有限，已于 2026-07-31 完整退役；后续传统 CRM 式提醒、流水和字段扩展不再推进。
...
**当前状态**：2026-05-20 曾完成当前环境验证；2026-07-31 按产品决定退役。历史迁移 `20260520_0016` 保留以维持迁移链，运行代码和入口已移除，`20260731_0079` 负责清理数据与 schema。
...
**退役实现**：删除 outbound 台账行与 `client_inquiry_events`，移除 `direction`、`stage`、`next_followup_at`、`cancelled_at`、`cancelled_by_id`、`cancel_reason`，并将保留的 inbound `first_response_time` 恢复为非空；任何异常关联 outbound 的报价任务会先解除咨询关联，报价任务本身保留。
...
- 已随 BIZ-1a 一并退役；未来如重启需重新立项。

#### BIZ-1c 跟进记录流水（暂停）

- 暂不启动。当前不继续扩展为 CRM，不新增跟进流水表或复杂客户生命周期管理。
- 已随 BIZ-1a 一并退役，不再依赖台账 `notes`。
...
**第一阶段（BIZ-1a 已退役，BIZ-2a 保留）**

| 子项 | 内容 | 状态 |
|------|------|------|
| 原 Phase 4a 运行态验收 | 历史会议纪要能力验收 | 已退役（2026-07-31） |
...
| BIZ-1a 台账 | 历史功能；`20260731_0079` 删除 outbound 数据、事件表和专用字段 | 已退役（2026-07-31） |
| COST-MEASURE 成本测算闭环 | 历史功能；`20260731_0080` 删除三张 `cost_measurement*` 专属表 | 已退役（2026-07-31） |
...
| 已退役 | 原 AI 平台 Phase 3 / Phase 4a 执行系统 | 2026-07-31 移除执行任务、执行速度看板与会议纪要；迁移 `20260731_0078` 删除专用表，项目进度不受影响。 |
| 已退役 | AI 平台升级 BIZ Track BIZ-1a：商务台账 v1 | 2026-07-31 移除入口、API/service/schema、事件模型、权限和开关；迁移 `20260731_0079` 清理 outbound 数据与专用 schema，Phase 2 inbound 客户咨询保留。 |
| 已退役 | 成本测算闭环（COST-MEASURE-1 / COST-MEASURE-2） | 2026-07-31 只移除 `cost_measurement*` 前后端链路、权限、开关与三张专属表；迁移 `20260731_0080` 执行 schema 清理，其他成本/报价/定额/预算计价/Agent 能力保留。 |
...
- [x] **原 AI 平台 Phase 3 / Phase 4a 执行系统已退役**：早期开发与验收记录保留用于历史追溯；2026-07-31 已移除执行任务、执行速度看板和会议纪要的运行代码、入口、功能开关与专用数据表。
- [x] **AI 平台升级 BIZ Track BIZ-1a 商务台账已退役**：2026-05-20 的历史验收记录保留用于追溯；2026-07-31 已移除运行代码、入口、功能开关和专用数据结构。
...
- [x] **统一报价双入口联昇工程量口径对齐（2026-07-28）**：旧对话入口的 Excel 自动标准化已复用预算项目工作簿语义预处理与工程量选择规则，正式清单只保留 `bill` 角色并排除主材参考表和重复表头；真实“联昇集团办公楼装饰工程清单.xlsx”只读内存复验为正式清单 198 行、有效工程量 198、缺失 0，装饰/机电/措施费分别取 K/J/E 列，分布 `121+73+4`。文本、图片输入和“账户定额 → 企业定额 → AI估价”三级报价顺序不变；无数据库结构变更、无业务数据写入。联合回归 `138 passed`，报价 Worker 重启后 `/health/ready=ready`。修正前旧报价任务不回写，需重新上传文件生成新任务；记录见 `AI_Middle_Office/docs/unified-quotation-chat-entry-correction-20260728.md`。
...
- [x] **成本预算对标 Phase 2 P2-2C-4：DeepSeek 批量估价稳定性调优（2026-07-17）**：针对联昇 198 行真实运行态中 `deepseek-v4-pro + ai_batch_size=12 + timeout=20s` 仍触发 ReadTimeout 与模型网关熔断的问题，新增预算估价专用 `BUDGET_PRICING_AI_MODEL`，当前 `.env` 配置为 `deepseek-v4-flash`，`BUDGET_PRICING_AI_TIMEOUT_SECONDS=45`；不再直接复用全局 `DEEPSEEK_MODEL`，避免影响投标等其它 DeepSeek 链路。一键生成报价默认批大小降为 `ai_batch_size=6`、并发提升为 `ai_concurrency=3`。后台任务失败策略改为延迟重试：普通批量异常先等待、拆小批、再小批/单行补跑，熔断最多等待 3 个恢复窗口后重试，达到上限才标记失败，减少前端 AI 失败快速上涨的体验问题。边界不变：只写可变计价草稿，不写正式 run、不写企业定额、不写账户定额 active。聚焦验证：`compileall` 通过，P2-2A/P2-2C `13 passed`，前端契约 `6 passed`，Vite build 通过；重启后生效。
- [x] **成本预算对标 Phase 2 P2-2C-3：DeepSeek 批量估价引擎（2026-07-17）**：将基础定额模式“一键生成报价”的 AI 估价从逐行请求升级为批量 JSON 引擎；新增 `generate_budget_pricing_ai_estimate_batch`，要求 DeepSeek 返回 `{"items":[...]}` 且每个输入 `row_id` 精确返回一次。后台任务默认 `ai_batch_size=12`、`ai_concurrency=2`，约把 198 行从 198 次请求降到约 17 次请求；前端创建任务时同步传入批大小和并发参数。批量失败时按批二分降级重试，缺失行会单行补跑；遇到模型网关熔断会等待 reset 后重试一次，避免本地熔断把后续大批行直接标失败。所有结果仍只写可变计价草稿，不写正式 run、不写企业定额、不写账户定额 active；旧 `partial_failed` 任务保留历史，新点击会创建新任务并复用已成功写入草稿的 AI 单价。聚焦验证：P2-2A/P2-2C `13 passed`，账户定额相邻专项 `11 passed`，前端契约 `6 passed`，Vite build 通过。运行态重启由用户手动执行；契约补充见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2c2-enterprise-ai-quote-job.md`。
- [x] **成本预算对标 Phase 2 P2-2C-2：基础定额模式“一键生成报价”后台任务（2026-07-17）**：新增 Alembic `20260717_0058`、`budget_project_pricing_draft_quote_jobs` / `budget_project_pricing_draft_quote_job_lines`、后台任务编排服务与三组 API：创建任务、查询当前任务、查询指定任务。基础定额模式会先创建/复用 `enterprise_ai` 草稿并匹配企业定额 active，未命中且无人工价/基础价的行进入 AI 队列；DeepSeek 调用不持有草稿/账号 DB 锁，写回时才短事务锁草稿和行，避免多行 AI 估价并发时的 lock wait timeout。前端新增“一键生成报价”按钮、进度条和运行中遮罩，任务终态后自动刷新草稿行；AI 失败行会落库并使任务进入 `partial_failed`，不让整单 500 消失。运行态发现真实 `SessionLocal.autoflush=False` 会导致任务行 add 后未 flush 即统计为 0 行，已补 `db.flush()` 并把专项测试改为 `autoflush=False` 保护；项目 15 的 4 条误判 0 行任务已标记 failed 作废，重新点击会创建新任务。当前环境已升级到 `20260717_0058 (head)`；聚焦验证：P2-2A/P2-2C `13 passed`，账户定额相邻专项 `11 passed`，前端契约 `6 passed`，Vite build 通过。正式 run、企业定额、账户定额 active 均不被写入；契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2c2-enterprise-ai-quote-job.md`。
- [x] **成本预算对标 Phase 2 P2-2C-1：单行 AI 估价进入计价草稿（2026-07-16）**：新增 `FEATURE_BUDGET_PRICING_AI_ESTIMATE`、预算计价 AI 估价配置、Alembic `20260716_0057` 的 `ai_estimated_unit_price` / `ai_estimate_snapshot_json`、单行估价服务与 `POST /pricing-draft/lines/{line_identifier}/ai-estimate`，前端草稿行新增“AI估价”按钮。AI 估价只写入可变草稿，不生成正式 run，不写企业定额，不写账户定额 active；有效价优先级为人工价 > AI 建议价 > 定额基础价，已有人工价或定额基础价的行禁止被 AI 覆盖。当前环境已启用开关并升级到 `20260716_0057 (head)`；项目 15 草稿 Rev9 -> Rev10，对“砌体墙拆除”写入 `ai_estimated_unit_price=46.750000`、`price_source=ai_estimate`、`line_total=88372.460000`，正式 run 仍为 4 个。0057 前备份 `162738186` bytes，SHA256 `2E147ADA820A48636C1C1CE0DC2107C77A566D36D3CD47356B41DB5802D0830A`；专项回归 `22 passed`、Vite build 通过。批量/自动估价尚未开始。契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2c1-ai-estimate-draft-line.md`。
- [x] **成本预算对标 Phase 2 P2-2B-3：账户定额 active 严格匹配（2026-07-16）**：账户定额模式现在仅读取当前账号的 `active` 条目，按定额编码或项目名称精确匹配且要求单位兼容，同名多规格仅在特征/规格唯一消歧时自动命中；未匹配、歧义、单位冲突均保持空价，不读取企业定额、不调用 LLM、不生成正式计价 run。新增 Alembic `20260716_0055` 的账户匹配证据字段；运行态发现同步审计行的 `RESTRICT` 草稿行外键会阻断可变草稿重建，已通过 `20260716_0056` 改为可空 `SET NULL`，保留不可变来源快照和版本审计。项目 15 草稿 Rev9 共 198 行，仅命中当前账号条目 2，单价 `50.000000`，`enterprise_quota_version_id=null`；当时数据库为 `20260716_0056 (head)`。专项回归 `19 passed`、Vite build 通过。后续 P2-2C-1 已单独完成，见上条。契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2b3-account-quota-active-matching.md`。
- [x] **成本预算对标 Phase 2 P2-2B-2：报价草稿同步到账户定额（2026-07-16）**：新增 `FEATURE_ACCOUNT_QUOTA_DRAFT_SYNC`、Alembic `20260716_0054`、`account_quota_sync_runs` / `account_quota_sync_lines`，以及计价草稿同步预览/确认 API。仅同步当前账号的正数人工改价行，确认时可新建、明确更新或跳过；新建/更新的账户定额一律为 `draft`，从不自动启用。每次确认写入同步批次、逐行来源草稿快照、目标条目、前后快照和处理结果；更新只改变价格、状态、revision，不覆盖用户维护的项目描述字段，归档条目不可更新。运行态已把项目 15 的“拆除铝合金玻璃门”人工价 `50.000000 元/㎡` 同步为 `pricing_draft_sync / draft / R1`，企业定额、正式计价 run 与来源草稿均未改变。P2-2B-3 账户定额匹配与 P2-2C LLM 估价未开始。契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2b2-account-quota-draft-sync.md`。
- [x] **成本预算对标 Phase 2 P2-2B-1：账户定额库底座（2026-07-16）**：新增 `FEATURE_ACCOUNT_QUOTAS`、Alembic `20260716_0053`、`account_quota_items` / `account_quota_item_history`、当前账号管理员 CRUD/状态/历史 API 和 Vite `/admin/account-quotas` 页面。账户定额按服务端当前账号隔离，API 不接受 `account_id`，跨账号访问按 404 处理；生命周期为 `draft <-> active -> archived`，archived 冻结；单价采用正数 `NUMERIC(18,6)` / Decimal，账号内以规范化 `名称 + 特征 + 规格 + 单位` SHA-256 指纹去重；人工 CRUD 只能写可信 `manual` 来源，每次编辑或状态流转递增 revision 并保存前后快照。0053 前备份为 `172461281` bytes，SHA256 `BFC58D4DB32AC39415B14181D513D90124A7A02237DF2296A76C0E4E7353BD46`；当前环境为 `20260716_0053 (head)`，`/health/ready=ready`，Celery `worker_count=2`。Chrome 完成 `R1 draft 123.456789 -> R2 130.000001 -> R3 active -> R4 draft -> R5 archived`，归档后仅保留历史，最终新表 `1 item / 5 history / manual / archived / revision 5`。受保护计数始终一致：企业定额 `3 versions / 1 active / 1422 items`，正式计价 `4 run / 792 line / 220 candidate / 4 event`，计价草稿 `1 draft / 198 line / 8 event`。联合专项 `29 passed`、前端契约 `5 passed`、Vite build `1627 modules`、compileall 通过；全量 `887 passed / 2 个冻结旧 BIZ-2c 失败 / 39 warnings`，无新增失败。P2-2B-2 账户定额匹配、P2-2B-3 同步到账户定额、P2-2C LLM 估价均未开始。契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2b1-account-quota-catalog.md`。
- [x] **成本预算对标 Phase 2 P2-2A：账号隔离 + 双模式计价草稿（2026-07-16）**：新增 `FEATURE_BUDGET_PRICING_DRAFTS`、Alembic `20260716_0052`、账号/成员/预算项目绑定和 account-scoped draft/line/event；API 从当前登录用户成员关系解析账号，不接受任意 `account_id`，管理员也只查看当前账号内项目。每个账号项目只有一个可变草稿，模式切换与人工改价只递增 revision，不生成正式 run。`enterprise_ai` 只读取严格唯一 active 企业定额，未匹配行保持 `NULL` 且本阶段不调用 LLM；`account_strict` 不读取企业定额，账户定额库未接入时全部价格保持 `NULL`。0052 前备份 `168194886` bytes（SHA256 `3D8E33324EADD446141EDBE2797E2617E56A4BA7908032DE3CCA5CE701660D84`），迁移回填 `account=1 / memberships=56 / budget bindings=12`。联昇 project 15、batch 16、revision 19 的草稿 `1 / ecb800c4-35cd-47f7-823f-aa077301b52f` 已完成 enterprise Rev1、manual Rev2（`12.345678 -> 790.37`）和 account_strict Rev3（198 行全 NULL）；正式 `3 run / 594 line / 165 candidate / 3 event` 未变。全量回归 `876 passed / 2 个冻结旧 BIZ-2c 口径失败`，Vite build 和 Chrome 验收通过。P2-2B 账户定额库、P2-2C AI 估价均未启动；契约见 `AI_Middle_Office/docs/cost-budget-benchmark-phase2-p2-2a-account-dual-mode-draft.md`。
- [x] **成本预算对标 Phase 2 P2-1：企业定额计价底座（2026-07-15）**：新增 `FEATURE_BUDGET_PRICING`、Alembic `20260716_0051`、不可变 run/line/candidate/event、严格唯一 `enterprise_quota active` 门禁、Decimal 六位计价、partial/complete 语义、只读候选证据和预算项目计价页。联昇项目 15 固定批次 16/revision 19/定额版本 3，已形成 3 个不可变 run；最终 run 3 是 run 2 的 child，并通过数据库统一时钟验证审计时间顺序。正式清单 198 行；当前定额健康可用 65/474 条，每个 run 均为 `matched=0 / ambiguous=14 / unmatched=182 / unit_conflict=2`、候选 55、覆盖率 0%、`total_cost=NULL`，正确暴露定额覆盖不足且不回退旧 `cost_items`。22 张受保护业务表计数未变；最终聚焦审计 `47 passed`、全量回归 `868 passed / 2 个冻结旧 BIZ-2c 口径失败` 且无新增失败，前端 build、后端 compileall 和 Chrome run 2 运行态验收通过。下一步 P2-2 仅允许人工选择 active 企业定额候选并创建 child run。
- [x] **成本预算对标 Phase 1 P1：预算项目与清单标准化底座收口（2026-07-15）**：新增 `FEATURE_BUDGET_PROJECTS`、Alembic `20260714_0049` / `20260715_0050`、不可变 revision、`parsed -> confirmed -> active -> superseded` 生命周期、项目活动批次/revision 双指针、409 并发保护、对象 capabilities 和统计不污染保护。信达 127 项空工程量安全置 0 的原验收事实保留；联昇运行态项目 `15 / PRJ-20260715-003` 共 8 个 Sheet、434 行，正式清单 198=`121+73+4` 且 `valid=198 / zero=0 / missing=0`，主材参考 99=`86+11+2`，取 `K/J/E`，F78/F80 公式错误留证，row 74 名称完整且全表名称不一致为 0。活动批次 16（UUID `5bdcd778-1d70-40e2-9206-ccf23d575ca1`）、initial revision 18、confirmed revision 19；confirm/activate 200，confirmed remap 409 frozen、stale remap 409 conflict；第二批次 17 保持 `parsed` 且活动指针仍为 16/19，项目内恰好 1 个 active 批次，事件覆盖 `parsed -> confirmed -> active`；批次 15 是预修复 `parsed` 未启用记录，项目保留未归档。0050 前已备份，迁移/运行态验收后 16 张受保护表逐项未变，企业定额仍恰好 1 个 `active`；P1 聚焦 `42 passed`、统计不污染 `19 passed`、前端契约 `30 passed`、前端 build 通过、后端全量 `839/841`，另 2 项为冻结旧 BIZ-2c 非阻断遗留。Phase 1 P1 已完成并可进入 Phase 2 数据契约/方案设计，但不自动授权计价实现，仍需用户决定。边界：企业定额唯一 `active` 是未来预算链唯一正式成本主库，本 P1 零条目读取、零写入、零匹配且只校验版本数量；旧 `cost_items` 报价/RAG 链冻结不得复用；“项目测算/采购结果”冻结；不接计价、定额匹配或四库；原件仍为 `metadata-only`。运行态证据见 `AI_Middle_Office/output/pre_budget_0050_20260715_p1_closeout/runtime_acceptance_project15.json`，交接详见 `AI_Middle_Office/docs/cost-budget-benchmark-phase1-budget-project-foundation.md`。
