...
readonly BACKUP_DIR="${BACKUP_ROOT}/pre-trusted-host-repair-${STAMP}"
readonly REPORT="/home/aiadmin/ai-phase6-trusted-host-repair-${STAMP}.txt"
...
rollback_and_exit() {
    ...
    if [[ ${compose_rc} -eq 0 && ${health_rc} -eq 0 ]]; then
        echo "ROLLBACK|original_env_restored_and_containers_recreated" >&2
    else
        echo "ROLLBACK_ERROR|manual_recovery_required|${BACKUP_DIR}" >&2
    fi
    exit 1
}
...
if [[ "${api_working_dir}" != "${COMPOSE_DIR}" \
    || "${api_config_files}" != "${COMPOSE_FILE}" ]]; then
    echo "ERROR|unexpected_compose_project_metadata" >&2
    exit 1
fi

trusted_line_count="$(grep -Ec '^TRUSTED_HOSTS=' "${ENV_FILE}" || true)"
if [[ "${trusted_line_count}" -ne 1 ]]; then
    echo "ERROR|trusted_hosts_line_count|${trusted_line_count}" >&2
    exit 1
fi

if grep -E '^TRUSTED_HOSTS=.*(^|,)[[:space:]]*\*([[:space:]]*,|[[:space:]]*$)' \
    "${ENV_FILE}" >/dev/null; then
    echo "ERROR|trusted_hosts_wildcard_forbidden" >&2
    exit 1
fi

if grep -E "^TRUSTED_HOSTS=.*(^|,)[[:space:]]*${DOMAIN//./\\.}([[:space:]]*,|[[:space:]]*$)" \
    "${ENV_FILE}" >/dev/null; then
    echo "ERROR|trusted_host_domain_already_present" >&2
    exit 1
fi
...
cp -a "${COMPOSE_FILE}" "${BACKUP_DIR}/compose.yaml"
{
    ...
    echo "api_image=$(docker inspect --format '{{.Config.Image}}' "${api_id}")"
    echo "worker_image=$(docker inspect --format '{{.Config.Image}}' "${worker_id}")"
    echo "api_started=$(docker inspect --format '{{.State.StartedAt}}' "${api_id}")"
    echo "worker_started=$(docker inspect --format '{{.State.StartedAt}}' "${worker_id}")"
    printf '%s\n' "${active_job_output}"
...
chmod 0600 "${BACKUP_DIR}/safe-state.before.txt"
sha256sum "${BACKUP_DIR}/app.env" "${BACKUP_DIR}/compose.yaml" \
    "${BACKUP_DIR}/safe-state.before.txt" > "${BACKUP_DIR}/SHA256SUMS"
chmod 0600 "${BACKUP_DIR}/SHA256SUMS"
echo "PASS|pre_change_backup"

temp_env="$(mktemp --tmpdir=/etc/ai-middle-office .app.env.trusted-host.XXXXXX)"
...
' "${ENV_FILE}" > "${temp_env}"; then
    mv "${temp_env}" "${BACKUP_DIR}/failed-app.env.tmp"
    chmod 0600 "${BACKUP_DIR}/failed-app.env.tmp"
    echo "ERROR|trusted_hosts_atomic_edit_failed" >&2
    exit 1
fi
...
if ! TARGET_ENV_FILE="${ENV_FILE}" TARGET_DOMAIN="${DOMAIN}" python3 -c '
import os
from pathlib import Path

lines = Path(os.environ["TARGET_ENV_FILE"]).read_text(encoding="utf-8").splitlines()
...
valid = (
    os.environ["TARGET_DOMAIN"] in hosts
...
'; then
    rollback_and_exit "updated_env_validation_failed"
fi
echo "PASS|trusted_host_env_updated"

if ! compose_recreate; then
    rollback_and_exit "api_worker_recreate_failed"
fi
echo "PASS|api_worker_recreated"

if ! wait_for_application; then
    rollback_and_exit "application_readiness_failed_after_recreate"
fi
...
worker_id="$(docker ps -q \
    ...
    --filter label=com.docker.compose.service=worker | head -n 1)"
if [[ -z "${api_id}" || -z "${worker_id}" ]]; then
    rollback_and_exit "containers_missing_after_recreate"
fi
...
raise SystemExit(0 if valid else 1)
'; then
    rollback_and_exit "runtime_trusted_hosts_validation_failed"
fi

if ! docker exec "${api_id}" sh -lc 'test "${PUBLIC_ACCESS_ENABLED:-}" = false'; then
    rollback_and_exit "public_access_boundary_changed"
fi
...
if [[ "${api_ip}" != "10.240.10.10" || "${worker_ip}" != "10.240.10.11" ]]; then
    rollback_and_exit "fixed_container_ip_changed"
fi
...
case "${domain_code}" in
    ...
    *) rollback_and_exit "domain_host_still_rejected|http_${domain_code}" ;;
esac
...
if [[ "${raw_owned}" -ne 3 || "${docker_owned}" -ne 2 ]]; then
    rollback_and_exit "private_forward_rule_counts_changed"
fi

if [[ "$(systemctl is-active nginx 2>/dev/null || true)" == "active" ]]; then
    rollback_and_exit "nginx_started_unexpectedly"
fi
if ss -lntH 'sport = :443' 2>/dev/null | grep -q .; then
    rollback_and_exit "port_443_started_listening_unexpectedly"
fi
