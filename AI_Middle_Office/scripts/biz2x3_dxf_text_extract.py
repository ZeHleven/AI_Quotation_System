...
def main() -> int:
    parser = argparse.ArgumentParser(description="BIZ-2x-3 DXF 图纸文本、图层和 layout 轻量提取")
    ...
    parser.add_argument(
        ...
        help="提取报告输出目录，默认 outputs/biz2x3",
    )
    parser.add_argument(
        "--text-record-limit",
        type=int,
        default=DEFAULT_TEXT_RECORD_LIMIT,
        help="每个 DXF 最多保存的文字记录数",
    )
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "DXF_TEXT_EXTRACTION_FAILED",
                    "message": str(exc),
                },
                ensure_ascii=False,
                indent=2,
    ...
    if not args.no_write:
        ...
        outputs = write_dxf_extraction_outputs(
            parsed_files,
            args.output_dir,
            stem=f"BIZ2x3_DXF图纸文本图层提取_{timestamp}",
        )
